...
static u_char *redis_ip = NULL, *block_second, *cookie_max = NULL;

static redisContext *c;
static redisReply *reply, *reply_cookie_max;
...
{ ngx_string("cookie_limit_req_zone"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE6,
		ngx_http_limit_req_zone, 0, 0, NULL },

{ ngx_string("cookie_limit_req"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF
		| NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_TAKE123,
		ngx_http_limit_req, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },

{ ngx_string("cookie_limit_req_log_level"), NGX_HTTP_MAIN_CONF
		| NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
		ngx_conf_set_enum_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(
				ngx_http_limit_req_conf_t, limit_log_level),
		&ngx_http_limit_req_log_levels },

{ ngx_string("cookie_limit_req_status"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF
...
static ngx_http_module_t ngx_http_cookie_limit_req_module_ctx = { NULL, /* preconfiguration */
...
ngx_module_t ngx_http_cookie_limit_req_module = { NGX_MODULE_V1,
		&ngx_http_cookie_limit_req_module_ctx, /* module context */
...
static ngx_int_t ngx_http_limit_req_handler(ngx_http_request_t *r) {
	...
	lrcf = ngx_http_get_module_loc_conf(r, ngx_http_cookie_limit_req_module);
	...
	const char *fmt_base = "%.*s";
	 (void)	snprintf(Host, sizeof(Host), fmt_base,
	...
	if (cookies != NULL && !c->err && c != NULL) {
		...
		int i;
		for (i = 0; i < 16; i++) {
			sprintf((char *) md5_buf2 + i * 2, "%02X", md5_buf[i]);
		}


		redisCommand(c, "pfadd [%s]Independent_IP %s", Host, (char *)md5_buf2);
		reply_cookie_max = redisCommand(c, "PFCOUNT [%s]Independent_IP", Host);
		if (reply_cookie_max->integer > atoi((char *)cookie_max)) {
			redisCommand(c, "SETEX %s %s %s", Host, block_second, Host);
	...
	for (n = 0; n < lrcf->limits.nelts; n++) {
		...
		if (rc && strncmp((char *) Host, "127.0.0.1", 9) != 0 && cookies != NULL) {
			if (!c->err) {
				reply = redisCommand(c, "GET white%s", (char *)md5_buf2);
				if (reply->str == NULL) {
					reply = redisCommand(c, "SETEX %s %s %s", (char *)md5_buf2,
							block_second, (char *)md5_buf2);

					/* Increase the history record  */
					reply = redisCommand(c, "SELECT 1");
					reply = redisCommand(c, "SET %s %s", (char *)md5_buf2, (char *)md5_buf2);
	...
	if (!c->err) {
		...
		reply = redisCommand(c, "GET %s", (char *) md5_buf2);
		reply_cookie_max = redisCommand(c, "GET %s", Host);
		ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
				"cookie_host [%s] [%s] [%s] [%s]", Host,
				       reply_cookie_max->str ? reply_cookie_max->str : "cookie_max empty",
				(char *)md5_buf2, reply->str ? reply->str : "query md5 empty");
		if (reply->str == NULL && reply_cookie_max->str == NULL) {
			freeReplyObject(reply);
			freeReplyObject(reply_cookie_max);
			return NGX_DECLINED;
		}
		if (reply->str != NULL && reply_cookie_max->str != NULL) {
			if (!ngx_strcmp((char * )Host,
					reply_cookie_max->str) && !ngx_strcmp((char * )Host, reply_cookie_max->str)) {
				ngx_log_error(lrcf->limit_log_level, r->connection->log, 0,
						"The number of cookies generated by the host has reached the limit: by zone \"%V\" Host=%s cookie_max=%d",
						&limit->shm_zone->shm.name, Host, atoi((char *)cookie_max));
		...
		if (reply->str == NULL) {
			if (!ngx_strcmp((char * )Host, reply_cookie_max->str)) {
				ngx_log_error(lrcf->limit_log_level, r->connection->log, 0,
									"The number of cookies generated by the host has reached the limit: by zone \"%V\" Host=%s cookie_max=%d",
									&limit->shm_zone->shm.name, Host, atoi((char *)cookie_max));
			...
			if (!ngx_strcmp((char * )md5_buf2, reply->str)) {
				ngx_log_error(lrcf->limit_log_level, r->connection->log, 0,
						"limiting cookie requests, excess: %ui.%03ui by zone \"%V\" lock=%s",
						excess / 1000, excess % 1000,
						&limit->shm_zone->shm.name, (char * )md5_buf2);
...
ngx_http_limit_req_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
	...
	for (i = 2; i < cf->args->nelts; i++) {
		...
		if (ngx_strncmp(value[i].data, "cookie_max=", 11) == 0) {

				len = value[i].len;
				cookie_max = value[i].data + 11;
	...
	shm_zone = ngx_shared_memory_add(cf, &name, size,
			&ngx_http_cookie_limit_req_module);
	if (shm_zone == NULL) {
		return NGX_CONF_ERROR ;
	}
